...
- **0.33.2** — `db.remove` no longer rewrites other nodes' edge lists: a removed node's id stays in them as an inert reference until `db.unlink`. If your UI lists `edges`, filter them against existing nodes.
