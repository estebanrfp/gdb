...
- **0.36.0** — the release that ships with its cryptographic specification ([CRYPTOGRAPHY.md](CRYPTOGRAPHY.md)); four formats move with it, and only the catch-up containers change on the wire. (1) Passkeys: the private key is wrapped under HKDF of the authenticator's PRF secret and stored under new keys, so a passkey protected by an earlier release is not seen — sign in with the phrase and `Protect with passkey` once; an authenticator without PRF is refused, and `sm: { resume: false }` keeps the secret out of the tab. (2) Encrypted records (`db.sm.put`): envelopes are format v2 and records sealed before do not decrypt — re-save durable encrypted data once, as in 0.28.0. (3) Fields sealed with `encryptDataForCurrentUser` are format v3; an earlier field is refused by its tag — re-save it once. (4) Catch-up envelopes and the hello are containers, never signed and without a clock: update a room's peers together and redeploy the Fallback Server (0.12.0).
