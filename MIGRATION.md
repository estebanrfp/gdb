...
Changes between engine versions are documented in the [CHANGELOG](CHANGELOG.md). Run one version per room, and the latest: every release is the only one that carries every fix, and since 0.34.0 peers on different versions refuse each other's live writes until they rejoin — an app loading the engine from the CDN at `@latest` needs nothing, an app bundling it from npm updates every build of the room together. One caution for CDN apps: the engine resolves its plugins (`sm.min.js` and the rest) beside the URL it was loaded from, and a browser caches each file for a week — a cache-buster on `index.min.js` alone refreshes the core and leaves a cached plugin of the previous release beside it. Load `@latest` plain, or pin a version (`genosdb@0.36.0/dist/index.min.js`) and bump it with the engine. Releases that ask for more than updating:

- **0.36.0** — the release that ships with its cryptographic specification ([CRYPTOGRAPHY.md](CRYPTOGRAPHY.md)); four formats move with it, and only the catch-up containers change on the wire. The engine reads each format in its 0.36.0 form only, so data sealed by an earlier release is carried across explicitly, before a room moves: (1) **Passkeys.** The private key is wrapped under HKDF of the authenticator's PRF secret and stored under new keys; a passkey registered by an earlier release is not read, and what that release stored stays on the device until the app removes it: `localStorage.removeItem('gdb_ethereum_material_encrypted_webauthn_v2')` and the same for `gdb_webauthn_registration_details_v2`, once, at startup — the engine neither reads nor clears them, and on a device whose passkey had no PRF that pair still unwraps the old key. Each person signs in with their phrase and chooses `Protect with passkey` once — the phrase is the only way back into an identity, so make sure every user holds theirs before the room moves. An authenticator without PRF is refused, and `sm: { resume: false }` keeps the secret out of the tab. (2) **Encrypted records** (`db.sm.put`) are format v2 and (3) **fields sealed with `encryptDataForCurrentUser`** are format v3: what an earlier release sealed does not open on 0.36.0, and 0.36.0 cannot re-save what it cannot read. Carry durable records across in two steps — read them on the release that sealed them (a page or script pinned to `genosdb@0.35.2/dist/index.min.js`, `db.sm.get` and `decryptDataForCurrentUser` return the plaintext), then write them again on 0.36.0 with `db.sm.put` and `encryptDataForCurrentUser`. Records that are disposable — demo rooms, session vaults, anything an app rebuilds — need nothing. (4) **Catch-up envelopes and the hello are containers**, never signed and without a clock: update a room's peers together and redeploy the Fallback Server (0.12.0). Apps that load `@latest` take the release as their visitors' caches expire, within a week: do the carrying-across on the app's own schedule, with the version pinned until it is done.
- **0.35.2** — a role node its subject rewrote travels as two operations, the superadmin's decision then the rewrite, and the receipt formerly attached to the rewrite is gone: a 0.35.2 peer learns such a node only from 0.35.2 peers. Update a room's peers together and redeploy the Fallback Server (0.11.12); nothing to migrate in stored data.
