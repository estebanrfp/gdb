...
- **0.36.0** — new API, nothing to migrate: `db.sm.sign(value)` and `db.sm.verify(envelope, maxAge?)` sign a value for the ephemeral channel and name its author. The graph is for facts; a signed envelope says who is saying something now — it labels, it never decides nor persists. Nothing existing changes.
