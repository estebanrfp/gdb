...
- **Optional Fallback Server (GenosSRV):** A zero-dependency superpeer (`dist/genossrv.min.js`) that keeps the room always available — durable SQLite memory, catch-up for returning peers, 24/7 governance authority, and an embedded signaling relay for fully sovereign deployments. One file, one command, one-click deployable. ([Fallback Server](https://github.com/estebanrfp/gdb/blob/main/docs/genosdb-fallback-server.md))
...
- **Completed Features**:
  ...
  - **Fallback Server (GenosSRV):** Optional always-on superpeer shipped with the dist as a single zero-dependency file — durable memory (SQLite WAL), availability anchor for peers that reconnect, both transports (full mesh and Cellular Mesh), and an optional superadmin identity that runs the governance engine **24/7**: promotions, demotions and role expirations no longer depend on a superadmin keeping a tab open. It can also embed a **signaling relay** (`--relay`) — an ephemeral, schnorr-verified Nostr relay served on `$PORT` — so discovery itself runs on your infrastructure: the entire stack (signaling, persistence, governance) in one deployable file. Runs anywhere Bun runs, or deploys to Heroku in one click. ([Fallback Server](https://github.com/estebanrfp/gdb/blob/main/docs/genosdb-fallback-server.md))
...
For deployments that need guaranteed availability on top of that, the optional [Fallback Server](https://github.com/estebanrfp/gdb/blob/main/docs/genosdb-fallback-server.md) adds an always-on superpeer — and can even host the room's signaling relay — without centralizing anything.
