...
- **Optional Fallback Server (GenosSRV):** A zero-dependency superpeer (`dist/genossrv.min.js`) that keeps the room always available — durable SQLite memory, catch-up for returning peers, and optional 24/7 governance authority. One file, one command, one-click deployable. ([Fallback Server](https://github.com/estebanrfp/gdb/blob/main/docs/genosdb-fallback-server.md))
...
For deployments that need guaranteed availability on top of that, the optional [Fallback Server](https://github.com/estebanrfp/gdb/blob/main/docs/genosdb-fallback-server.md) adds an always-on superpeer — without centralizing anything.
