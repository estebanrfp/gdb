...
- **Full-state sync merges instead of replacing — peers no longer lose data when another device joins.** Receiving a full state discarded the entire local graph, replaced it with the sender's, and persisted the result. That is only safe when the sender is genuinely ahead, and the handshake could not tell: a peer decides "you are too far behind" by comparing your timestamp against the oldest entry in *its own* capped oplog, so a peer that had just written anything looked authoritative. A Security Manager login is enough to trigger it — the `user:0x…` bootstrap write lands in the new device's oplog with a fresh timestamp — so signing in on a second device shipped its nearly empty graph to the first one and wiped it, on disk, silently ([#34](https://github.com/estebanrfp/gdb/discussions/34)). Incoming state is now merged node by node with the Hybrid Logical Clock, exactly as `link` and `upsert` already resolved conflicts: newer wins, and nodes only the receiver holds survive. Three changes complete it — the exchange is **reciprocal** (a receiver holding nodes the sender lacks answers with its own state, so both converge instead of one side keeping the difference); deletions travel with it as **`removals`**, because the oplog already records every `remove` and persists them, which makes it a tombstone log at no extra cost — without it a merge cannot distinguish "absent" from "deleted", and a stale peer resurrects what you erased; and the `Local newer, ignoring full sync` short-circuit is gone, since it existed to blunt the destructive replace and now only discarded legitimate state. The oplog is kept after a full sync — it holds those tombstones — while the `clear()` in "delete the whole database" stays. Verified across two browsers on the ACLs + Governance testbed and a two-device repro: divergent writes converge, and a delete survives the other peer reconnecting with a stale copy. **Update every peer in a room:** the wire format is unchanged and mixed versions interoperate, but a peer still on an older build applies the old behavior on its side.
...
- **`oplogSize` now defaults to `200` (was `20`).** The window is no longer only a delta-sync optimization: after this release it is also how long a deletion is remembered, so a peer returning with a stale copy cannot resurrect it. Twenty operations were too few for either job — a normal editing session overruns it in minutes, dropping peers into the full-state path. The cost is paid per write, since the oplog is persisted on every operation: on a mass-insert benchmark the per-write cost moves from ~37 µs to ~65 µs. That difference is invisible at human interaction rates and matters only to bulk imports, which can still set a smaller value explicitly.
