...
*   **Access Control Lists (ACLs):**
    *   **Node-level permissions:** Beyond global roles, node owners grant/revoke per-node `read`/`write`/`delete` access to specific users via `db.sm.acls.set` / `grant` / `revoke`. These are enforced against **malicious peers** — the cryptographically-verified author is checked on every write, whether it arrives as a live operation or through state reconciliation, so a modified peer cannot write a node it does not own.
