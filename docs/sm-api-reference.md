...
When the Security Manager initializes, it resumes a WebAuthn-backed session silently if the previous session on this origin was completed with WebAuthn and its secret is still held by the tab. Reloads resume without a prompt; a new tab or browser session requires `db.sm.loginCurrentUserWithWebAuthn()`. With `sm: { resume: false }` the tab keeps no secret at all and every page load asks the authenticator.
...
Encrypts a **value** instead of a whole record. The key derives from the active user's private key (HKDF-SHA-256 → AES-256-GCM), so it encrypts *for yourself*: there is no recipient and it cannot share data with another user — to share an encrypted **record**, use `db.sm.put` + `db.sm.acls.grant` instead.
...
- **Returns**: `{Promise<string>}` – `{"iv":…,"encrypted":…,"type":"aes-gcm-self-ssm-v3"}`. Being a string, it stores as an ordinary node field. `decrypt` returns the original value.
- **Throws** (decrypt, one message per cause): `User session (signer) required` · `encryptedString is not valid JSON` · `Unsupported encryption type or incorrect version` (a field sealed by an earlier format — re-save it) · `Invalid encrypted data format` · **`Failed to decrypt personal data`** when the data belongs to someone else.
...
Initiates the WebAuthn registration process to protect an Ethereum private key. The private key is encrypted under a key derived from the authenticator's PRF secret; only the ciphertext and the credential id are stored in localStorage. An authenticator without the PRF extension is refused: the call returns `null`, nothing is stored, and the session continues by mnemonic.
