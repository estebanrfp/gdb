...
When the Security Manager initializes, it resumes a WebAuthn-backed session silently if the previous session on this origin was completed with WebAuthn and its secret is still held by the tab. Reloads resume without a prompt; a new tab or browser session requires `db.sm.loginCurrentUserWithWebAuthn()`. On authenticators without PRF the secret is device-bound and every load resumes.
...
Initiates the WebAuthn registration process to protect an Ethereum private key. The private key is encrypted with the authenticator's PRF secret; only the ciphertext and the credential id are stored in localStorage.
