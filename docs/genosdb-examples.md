...
A shared code editor and a code host in one page, for projects that are one HTML file — HTML, CSS and JavaScript together — with no server. The editor is the [block editor](https://estebanrfp.github.io/gdb/examples/block-editor.html)'s technique applied to code: **one node per line**, edited live by everyone on the branch with named carets, line numbers derived, and the page running beside the code as you type. Underneath, the history is owned: a commit is a node its author owns under an id that carries the author's address, a branch head moves only for its owner and the addresses granted `write`, a pull request is a proposal whose merged state every peer reads from the graph, merges are line-based three-way merges that the shared buffer follows, and every commit runs from the timeline. Writing is free from the first second, under a constitution every peer runs. The article: [A GitHub With No Server](https://genosdb.com/dcode-p2p-collaborative-code-editor-no-server).
