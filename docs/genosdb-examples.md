...
A continuous document — the Notion model — over the plain GenosDB API. One node per paragraph, so block-level LWW means edits on different paragraphs never collide; fractional order keys ([how they work](ordered-lists.md)), so concurrent inserts into the same gap both survive, identically ordered everywhere; Enter splits a paragraph into two nodes, Backspace at its start merges them back, Alt+↑↓ moves one with a single `put`. Beside the document, the keys themselves on a number line, to scale: Enter drops a dot into the gap under the caret and the panel says how many halvings that gap has left, a paste fans its lines across one gap at once, a move slides a dot while its id stays. What Yjs calls *awareness* rides one ephemeral GenosRTC channel: named colour carets, live selections, and the paragraph being typed broadcast keystroke by keystroke — the character-by-character experience of a CRDT editor, while the debounced graph write remains the truth that persists and repairs. No server and no CRDT library; measured beside the public Yjs/Automerge demos, edits land with less latency and less work per keystroke.

### [Keystrokes — Interleaving Inside One Span, Where an App Wants It](https://estebanrfp.github.io/gdb/examples/keystrokes.html)
One paragraph everyone types into at once, inside the same word if they like — the case a plain database is said not to cover — with no CRDT. A keystroke travels the ephemeral channel as an edit at a position of the last snapshot the graph confirmed, stamped; every window renders base + edits in the same order, so the letters interleave identically everywhere; a pause writes the paragraph with `db.put`, the snapshot becomes every window's new base, and two snapshots that cross are the engine's business. The right panel shows the two layers as they happen — the channel keystroke by keystroke, the graph snapshot by snapshot — and a hash of the text each window shows, the proof that they agree. The mechanism an application adds where it needs keystroke-level interleaving, and only there.

_By: [Esteban Fuster Pozzi (estebanrfp)](https://github.com/estebanrfp)_

### [Spreadsheet — Rows and Columns With Identities](https://estebanrfp.github.io/gdb/examples/spreadsheet.html)
A shared sheet over the plain GenosDB API. Rows and columns are identities whose position is a fractional key ([how they work](ordered-lists.md)), and a cell is a node named after its row and its column: move a row and its cells go with it in one `put`, insert a column and no data shifts, sort by a column and the rows are re-keyed with their ids intact. The base grid is never written — a row nobody inserted has the same id and key on every peer — so two people who type into `A3` of an empty sheet without seeing each other write the same cell and end with one value, not six rows and two cells: the empty-sheet problem of Bartosz Sypytkowski's spreadsheet post, solved by construction. Which cell every peer stands on rides an ephemeral channel. Beside the sheet, the keys themselves: every row on a number line down the side and every column across the top, to scale — a hollow dot is a name with no node behind it, a filled one has a node — with the gap beside the selected cell and how many halvings a float64 has left in it. Under the keys, the graph write by write: every node as it lands, with the peer that wrote it and its clock stamp — a sort shows as row writes and not one cell write, and two peers on one cell as two stamps on one name.

### [Outliner — Every Bullet Is a Node, Every Indent an Edge](https://estebanrfp.github.io/gdb/examples/outliner.html)
A Workflowy-style outliner where the tree is the graph itself: each bullet is a node, its children are its edges, and moving a branch is an `unlink` plus a `link` — nothing is re-serialized, however deep the branch, and the other peers receive two edge changes instead of a document. Siblings order by a fractional rank ([how it works](ordered-lists.md)), so concurrent inserts into the same gap both survive; Enter splits a bullet, Backspace merges it into the one above, and only a leaf can be deleted, so a removal never orphans a subtree. Zoom into any bullet and the search runs `$edge` from it, reading its descendants at any depth in one query. Beside the outline, the same data drawn as the graph it is stored as — a dot per node, a line per edge, every edge, laid out by a force simulation — so moving a branch shows as the branch swinging to its new parent while nothing else is re-sent.

### [Split Expenses — A Balance Is a Sum Over Everything That Arrived](https://estebanrfp.github.io/gdb/examples/split-expenses.html)
A group's shared expenses without accounts: the name of the group is the link, the link is the room, and whoever opens it is in. Each expense is one node — who paid, how much, split among whom — and nothing else is stored: every balance and every settle-up transfer is derived on the fly from the whole set, in integer cents, so a group's balances always sum to exactly zero. That makes it the most honest convergence demo in the catalogue: two windows either show the same balances or you can see they have not converged yet, because a balance is a sum over everything that arrived. Settling up writes one more node of the same kind, and the books close by themselves.
...
### [dCode](https://estebanrfp.github.io/dCode/) · [source](https://github.com/estebanrfp/dCode)
A shared code editor and a code host in one page, for projects that are one HTML file — HTML, CSS and JavaScript together — with no server. The editor is the [block editor](https://estebanrfp.github.io/gdb/examples/block-editor.html)'s technique applied to code: **one node per line**, edited live by everyone on the branch with named carets, line numbers derived, and the page running beside the code as you type. Underneath, the history is owned: a commit is a node its author owns under an id that carries the author's address, a branch head moves only for its owner and the addresses granted `write`, a pull request is a proposal whose merged state every peer reads from the graph, merges are line-based three-way merges that the shared buffer follows, and every commit runs from the timeline. A **private repository** keeps its key in a vault — one node encrypted by `db.sm.put`, an envelope per member granted and revoked by the owner — and every line and commit travels and rests as ciphertext, unreadable even to the authority. Writing is free from the first second, under a constitution every peer runs; the session pill opens an identity view where a phrase session takes a passkey later, and the theme follows the design guide. The article: [A GitHub With No Server](https://genosdb.com/dcode-p2p-collaborative-code-editor-no-server).
...
### [WebGPU World](https://estebanrfp.github.io/webgpu-world/) · [source](https://github.com/estebanrfp/webgpu-world)
A shared 3-D world whose entire multiplayer layer is one ~180-line file: type a name and everyone in the same room watches you walk, run and swim across an island, with no game server and no accounts. It is the counterpart to the other showcases here, because **nothing is written to the graph** — a position is worthless two frames later, so it travels as transient traffic on a single `db.room.channel` at 12 Hz, opened with `rtc: true` and **no Security Manager**: walking around a world does not need a signed identity, and the name you type is a label, not an account. The database name is the room, so `?room=<name>` is a private world; losing the connection costs you the other players, not the world, and with no CDN or relay reachable it degrades silently to single-player instead of failing. A remote player is rendered as an NPC driven by the network, taking a pre-allocated slot whose bind groups already exist, so **a peer joining allocates nothing on the GPU** while the render loop stays at 60 fps. The island it happens on is a proof of concept in its own right: terrain, ocean, weather, runtime-built voxel avatars and physics written **directly on the native WebGPU API**, hand-written WGSL, no 3-D engine. The Playwright suite runs **every peer in its own browser context and every test in its own room** — storage isolation alone does not isolate the network — and asserts the transport rather than the state: a `candidate-pair` in `succeeded` carrying bytes, because "it showed up in the other tab" proves nothing when tabs share storage and `BroadcastChannel`.
