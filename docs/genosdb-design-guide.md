...
> **[`examples/docs.html`](../examples/docs.html)** ([live](https://estebanrfp.github.io/gdb/examples/docs.html)) for the **full profile** — an application with identity, permissions and state — and **[`examples/traversal-depth.html`](../examples/traversal-depth.html)** ([live](https://estebanrfp.github.io/gdb/examples/traversal-depth.html)) for the **minimal profile**, the two-panel bench every tool page should follow (§5.6).
>
...
> **The reference implementation for a bench is [`examples/traversal-depth.html`](../examples/traversal-depth.html)** ([live](https://estebanrfp.github.io/gdb/examples/traversal-depth.html)) — the minimal-profile counterpart to `docs.html`. Query left, answer and trace right, one surface, floating theme toggle, no session and no chrome bands. Copy from the file; the text below explains why it is shaped that way.
...
- **To centre a short page, `place-items: center`, not `place-content`.** The latter centres the grid's *tracks*, and an implicit track sizes itself to the widest content — so a `width: min(460px, 100%)` block ends up left-aligned inside a wider track, vertically centred and horizontally not. Pair it with `min-height: 100dvh` rather than `height`, so a short window scrolls instead of clipping.
...
5. **A value that lands on the field being typed in lands merged.** When two peers write over the same value, the engine keeps both contributions — the writer whose op lost re-applies its edit over the winner ([Concurrent Writes](genosdb-concurrent-writes.md)) — and the editor must not undo it. Never overwrite the keystrokes not yet written, and never ignore the incoming value either: keep the text the graph last confirmed for that field, merge it with what is being typed and what arrived — one changed region per side, the engine's own rule — paint the result and shift the caret by what landed before it. The block editor's `updateBlock` is the reference. An editor that ignores incoming values while typing writes over the merge on its next save.
